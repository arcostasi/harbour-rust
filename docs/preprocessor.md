...
| `tests/fixtures/pp/define_clipboard_root.prg` | golden do subset focado de `DEFINE CLIPBOARD ... FORMAT TEXT` derivado da regra inline de `hbpptest.prg` |
...
| `tests/fixtures/pp/save_all_like_root.prg` | golden do primeiro degrau do bloco `SAVE`, cobrindo `SAVE ALL LIKE A TO A` e os dois smart markers `<(f)>` / `<(p)>` do `std.ch` |
| `tests/fixtures/pp/save_to_all_like_root.prg` | golden do degrau reordered irmão `SAVE TO A ALL LIKE A`, cobrindo a forma adjacente do mesmo `__mvSave( <(f)>, <(p)>, .t. )` em `std.ch` |
| `tests/fixtures/pp/save_to_all_root.prg` | golden do branch base `SAVE [TO <(f)>] [ALL]`, cobrindo `SAVE TO A ALL` com wildcard `"*"` no segundo argumento |
| `tests/fixtures/pp/save_all_except_root.prg` | golden do primeiro ramo `.f.` do bloco `SAVE`, cobrindo `SAVE ALL EXCEPT A TO A` antes da forma reordered |
...
| `tests/fixtures/pp/get_command_picture_range_when_caption_reordered_root.prg` | golden do subset focado de `@ <row>, <col> GET <var>` com o caso `@ 2,5 GET a PICTURE "X" RANGE 0,100 WHEN .T. CAPTION "myget"` |
| `tests/fixtures/pp/get_command_picture_range_when_caption_message_reordered_root.prg` | golden do subset focado de `@ <row>, <col> GET <var>` com o caso `@ 2,6 GET a PICTURE "X" RANGE 0,100 WHEN .T. CAPTION "myget" MESSAGE "mymess"` |
| `tests/fixtures/pp/get_command_picture_range_when_caption_message_send_reordered_root.prg` | golden do subset focado de `@ <row>, <col> GET <var>` com o caso `@ 2,7 GET a PICTURE "X" RANGE 0,100 WHEN .T. CAPTION "myget" MESSAGE "mymess" SEND send()` |
| `tests/fixtures/pp/get_command_pushbutton_base_root.prg` | golden do primeiro subset focado do bloco adjacente `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo apenas `@ 4,1 GET a PUSHBUTTON` |
...
| `tests/fixtures/pp/get_command_pushbutton_color_root.prg` | golden do sexto subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color()` |
| `tests/fixtures/pp/get_command_pushbutton_focus_root.prg` | golden do sétimo subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus()` |
| `tests/fixtures/pp/get_command_pushbutton_state_root.prg` | golden do oitavo subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state()` |
| `tests/fixtures/pp/get_command_pushbutton_style_root.prg` | golden do nono subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state() STYLE style()` |
| `tests/fixtures/pp/get_command_pushbutton_send_root.prg` | golden do décimo subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state() STYLE style() SEND send()` |
| `tests/fixtures/pp/get_command_pushbutton_guisend_root.prg` | golden do décimo primeiro subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state() STYLE style() SEND send() GUISEND guisend()` |
| `tests/fixtures/pp/get_command_pushbutton_size_root.prg` | golden do décimo segundo subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state() STYLE style() SEND send() GUISEND guisend() SIZE X 100 Y 100` |
| `tests/fixtures/pp/get_command_pushbutton_capoff_root.prg` | golden do décimo terceiro subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state() STYLE style() SEND send() GUISEND guisend() SIZE X 100 Y 100 CAPOFF X 10 Y 10` |
| `tests/fixtures/pp/get_command_pushbutton_bitmap_root.prg` | golden do décimo quarto subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state() STYLE style() SEND send() GUISEND guisend() SIZE X 100 Y 100 CAPOFF X 10 Y 10 BITMAP bitmap()` |
| `tests/fixtures/pp/get_command_pushbutton_bmpoff_root.prg` | golden do décimo quinto subset focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON VALID valid() WHEN when() CAPTION "cap" MESSAGE "mes" COLOR color() FOCUS focus() STATE state() STYLE style() SEND send() GUISEND guisend() SIZE X 100 Y 100 CAPOFF X 10 Y 10 BITMAP bitmap() BMPOFF X 2 Y 2` |
| `tests/fixtures/pp/get_command_pushbutton_color_only_root.prg` | golden do recorte adjacente focado do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON COLOR "W/N"` com apenas o slot `COLOR` preenchido no `_PushButt_` |
| `tests/fixtures/pp/get_command_pushbutton_reordered_sparse_root.prg` | golden do recorte adjacente seguinte do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON COLOR "W/N" SIZE X 100 Y 100 BMPOFF X 2 Y 2 VALID valid() GUISEND guisend() WHEN when() MESSAGE "mes"` com `COLOR`, `SIZE`, `BMPOFF`, `VALID`, `GUISEND`, `WHEN` e `MESSAGE` em ordem reordenada e slots intermediários ainda vazios no `_PushButt_` |
| `tests/fixtures/pp/get_command_pushbutton_reordered_sparse_color_tail_root.prg` | golden do recorte adjacente irmão do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON SIZE X 100 Y 100 BMPOFF X 2 Y 2 VALID valid() GUISEND guisend() WHEN when() MESSAGE "mes" COLOR "W/N"` com `COLOR "W/N"` deslocado para a cauda da permutação e a mesma expansão observável do upstream |
| `tests/fixtures/pp/get_command_pushbutton_reordered_full_root.prg` | golden do caso maior adjacente do mesmo bloco `@ <row>, <col> GET <var> PUSHBUTTON`, cobrindo `@ 4,1 GET a PUSHBUTTON SIZE X 100 Y 100 BMPOFF X 2 Y 2 VALID valid() GUISEND guisend() WHEN when() MESSAGE "mes" COLOR "W/N" CAPOFF X 10 Y 10 FOCUS focus() STATE state() STYLE style() SEND send() BITMAP bitmap() CAPTION "cap"` com a composição completa reordenada observada em `hbpptest.prg` |
| `tests/fixtures/pp/define_clipboard_root.prg` | golden do recorte adjacente seguinte de `hbpptest.prg`, cobrindo `DEFINE CLIPBOARD oC OF oD FORMAT TEXT` com `FORMAT <format:TEXT,OEMTEXT,BITMAP,DIF>` e `UPPER(<(format)>)` no replacement |
...
| `tests/fixtures/pp/get_command_caption_root.prg` | golden do quinto subset focado de `@ <row>, <col> GET <var>` com o caso `@ 0,5 GET a PICTURE "X" VALID .T. WHEN .T. CAPTION "myget"` de `hbpptest.prg` |
...
| `tests/fixtures/pp/get_command_valid_range_root.prg` | golden do terceiro recorte maior adjacente do bloco `GET`, cobrindo o caso observável `@ 1,3 GET a PICTURE "X" VALID .T. RANGE 0,100` onde o `RANGE` não sobrevive na expansão final do `hbpptest.prg` |
| `tests/fixtures/pp/get_command_when_range_root.prg` | golden do quarto recorte maior adjacente do bloco `GET`, cobrindo `@ 1,4 GET a PICTURE "X" WHEN .T. RANGE 0,100` com `RANGE` e `WHEN` sobrevivendo juntos na expansão final |
| `tests/fixtures/pp/get_command_caption_range_root.prg` | golden do quinto recorte maior adjacente do bloco `GET`, cobrindo `@ 1,5 GET a PICTURE "X" WHEN .T. CAPTION "myget" RANGE 0,100` com `RANGE`, `WHEN` e `CAPTION` |
| `tests/fixtures/pp/get_command_message_range_root.prg` | golden do sexto recorte maior adjacente do bloco `GET`, cobrindo `@ 1,6 GET a PICTURE "X" WHEN .T. CAPTION "myget" MESSAGE "mymess" RANGE 0,100` com `RANGE`, `WHEN`, `CAPTION` e `MESSAGE` |
...
- quinto subset focado do mesmo bloco `@ <row>, <col> GET <var>`, cobrindo `@ 0,5 GET a PICTURE "X" VALID .T. WHEN .T. CAPTION "myget"` antes de subir para `MESSAGE`
...
- terceiro recorte maior adjacente do mesmo bloco `GET`, cobrindo `@ 1,3 GET a PICTURE "X" VALID .T. RANGE 0,100` e registrando o comportamento observável do upstream em que o `RANGE` deixa de surtir efeito na expansão final
- quarto recorte maior adjacente do mesmo bloco `GET`, cobrindo `@ 1,4 GET a PICTURE "X" WHEN .T. RANGE 0,100` e confirmando a coexistência de `RANGE` com `WHEN`
- quinto recorte maior adjacente do mesmo bloco `GET`, cobrindo `@ 1,5 GET a PICTURE "X" WHEN .T. CAPTION "myget" RANGE 0,100` e confirmando a composição de `RANGE` com `WHEN` e `CAPTION`
- sexto recorte maior adjacente do mesmo bloco `GET`, cobrindo `@ 1,6 GET a PICTURE "X" WHEN .T. CAPTION "myget" MESSAGE "mymess" RANGE 0,100` e confirmando a composição de `RANGE` com `WHEN`, `CAPTION` e `MESSAGE`
