...
- sem markers avançados como `<{id}>`, além da semântica mais ampla de `<"id">` e `<(id)>`
...
- baseline de compatibilidade focado contra `doc/pp.txt`, `tests/hbpp/_pp_test.prg` e `tests/hbpp/hbpptest.prg`, incluindo replacements com `\[`/`\]`, reordenação selecionada de cláusulas opcionais contíguas, result markers lógicos `<.id.>`, um subset mínimo de `<"id">` e um subset mínimo de `<(id)>`
