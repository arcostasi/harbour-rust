...
O pré-processador já suporta o baseline alpha atual do projeto. Fixtures focadas de compatibilidade agora cobrem replacements opcionais com colchetes escapados, reordenação selecionada de cláusulas opcionais contíguas, um subconjunto focado de opcionais/listas nested derivado das regras `AAA`, `SET`, `AVG`, `INSERT` e `INSERT2` do upstream, incluindo as declarações multi-linha de `SET`/`AVG` exercitadas em `hbpptest.prg`, expansão repetida de cláusulas opcionais e continuação de source line para o subconjunto `INSERT`/`INSERT2`, um subconjunto focado de reordenação de cláusulas opcionais multi-linha derivado da regra upstream `_pp_test` `MYCOMMAND3`, um subconjunto focado de padrão e corpo multi-linha exercitado pelas regras upstream `INSERT2`, `MYCOMMAND2` e `MYCOMMAND3`, incluindo a declaração multi-linha de `MYCOMMAND2` e a permutação com `ALL` antes da lista, result markers lógicos como `<.id.>`, um subconjunto mínimo de blockify `<{id}>`, um subconjunto quoted-result orientado a macros para `<"id">`, um subconjunto smart-result orientado a macros para `<(id)>` e um subconjunto expandido de pattern marker de macro `<id:&>`, ancorados no corpus hbpp do upstream e no `pp.txt`, incluindo spillover selecionado em operadores, cadeias longas com múltiplos segmentos, misturas selecionadas com `&(expr)` e um subconjunto focado da sintaxe de match `XTRANS(<x>(` / `XTRANS(<x:&>(`, agora também com variantes de ponto, indexação, operadores lógicos, preservação literal em `hbpptest.prg` e cadeias concatenadas de macro em `_pp_test.prg`. Combinações mais amplas de opcionais/listas, edge cases de dumb-stringify, behavior mais amplo de pattern markers de macro e semântica mais ampla desses markers continuam como trabalho futuro.
