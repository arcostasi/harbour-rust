...
O repositório já possui uma matriz prática de testes cobrindo slices do compilador, comportamento de runtime, execução via CLI, snapshots golden, compare tooling, benchmark smoke e checks de compilação de fuzzing. O corpus de compatibilidade também inclui fixtures focadas de strings para comportamento de trim com caracteres de controle, edge cases de `At()`, recorte em `SubStr()`/`Left()`/`Right()`, overflow/preservação em `Replicate()` e parsing de `Val()` em casos com ponto final sem fração, pontuação repetida, sinais repetidos e entradas parecidas com expoente, ancoradas no oráculo do upstream quando disponível. O lado executável também inclui um harness dedicado de host C cobrindo a preservação de `Chr(0)` embutido em helpers selecionados de string, para exercitar essa camada além da surface apenas em Rust.
