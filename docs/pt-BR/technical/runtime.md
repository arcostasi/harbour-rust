...
- slices focados da fase 16 em runtime/biblioteca agora cobrem `hb_JsonDecode()`, `hb_gzCompressBound()`, a superfície atual de `hb_gzCompress()` incluindo writeback de buffer de destino, `hb_processRun( cCommand )` e captura de stdout com `hb_processRun( cCommand, NIL, @cStdOut )`.
