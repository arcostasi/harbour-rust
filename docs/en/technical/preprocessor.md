...
The preprocessor already supports the project's current alpha baseline. Focused compatibility fixtures now cover escaped-bracket optional replacements, selected reordering across contiguous optional clauses, selected logical result markers such as `<.id.>`, a minimal blockify-result subset for `<{id}>`, a minimal quoted-result subset for `<"id">`, and a minimal smart-result subset for `<(id)>` drawn from the upstream hbpp corpus. Broader nested optional/list combinations, dumb-stringify edge cases, and broader marker semantics still remain future work.
