...
- focused phase 16 runtime-library slices now cover `hb_JsonDecode()`, `hb_gzCompressBound()`, `hb_ZCompressBound()`, the current `hb_gzCompress()` surface including destination-buffer writeback, `hb_ZError()`, `hb_processRun( cCommand )`, and `hb_processRun( cCommand, NIL, @cStdOut )` stdout capture.
