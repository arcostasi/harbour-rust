...
#[test]
fn phase15_list_destination_fixture_matches_curated_upstream_subset() {
    let Some(upstream_pp_test) = read_upstream_or_skip(
        "harbour-core/tests/hbpp/_pp_test.prg",
        "upstream _pp_test corpus",
    ) else {
    ...
    let expected = fs::read_to_string(workspace_fixture(
        "tests/fixtures/pp/list_destination_root.out",
    ))
    .expect("fixture snapshot");

    assert!(upstream_std.contains("#command LIST [<v,...>] [<off:OFF>]"));
    assert!(upstream_std.contains("[<prn:TO PRINTER>] [TO FILE <(f)>]"));
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST TO PRINTER")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST TO FILE a")
    );

    let output = Preprocessor::default().preprocess(
        SourceFile::from_path(workspace_fixture(
            "tests/fixtures/pp/list_destination_root.prg",
        ))
        .expect("fixture"),
    );
...
#[test]
fn phase15_list_off_fixture_matches_curated_upstream_subset() {
    let Some(upstream_pp_test) = read_upstream_or_skip(
        "harbour-core/tests/hbpp/_pp_test.prg",
        "upstream _pp_test corpus",
    ) else {
    ...
    let expected = fs::read_to_string(workspace_fixture("tests/fixtures/pp/list_off_root.out"))
        .expect("fixture snapshot");

    assert!(upstream_std.contains("#command LIST [<v,...>] [<off:OFF>]"));
    assert!(upstream_pp_test.lines().any(|line| line == "LIST OFF"));
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST OFF TO PRINTER")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST OFF TO FILE a")
    );

    let output = Preprocessor::default().preprocess(
        SourceFile::from_path(workspace_fixture("tests/fixtures/pp/list_off_root.prg"))
            .expect("fixture"),
    );
...
#[test]
fn phase15_list_fields_fixture_matches_curated_upstream_subset() {
    let Some(upstream_pp_test) = read_upstream_or_skip(
        "harbour-core/tests/hbpp/_pp_test.prg",
        "upstream _pp_test corpus",
    ) else {
    ...
    let expected = fs::read_to_string(workspace_fixture("tests/fixtures/pp/list_fields_root.out"))
        .expect("fixture snapshot");

    assert!(upstream_std.contains("__dbList( <.off.>, { <{v}> }, .t.,"));
    assert!(upstream_pp_test.lines().any(|line| line == "LIST a"));
    assert!(upstream_pp_test.lines().any(|line| line == "LIST a,b"));
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a,b,(seek(a+b),c)")
    );

    let output = Preprocessor::default().preprocess(
        SourceFile::from_path(workspace_fixture("tests/fixtures/pp/list_fields_root.prg"))
            .expect("fixture"),
    );
...
#[test]
fn phase15_list_fields_destination_fixture_matches_curated_upstream_subset() {
    let Some(upstream_pp_test) = read_upstream_or_skip(
        "harbour-core/tests/hbpp/_pp_test.prg",
        "upstream _pp_test corpus",
    ) else {
    ...
    let expected = fs::read_to_string(workspace_fixture(
        "tests/fixtures/pp/list_fields_destination_root.out",
    ))
    .expect("fixture snapshot");
    ...
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a TO PRINTER")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a TO FILE a")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a,b TO PRINTER")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a,b,(seek(a+b),c) TO FILE a")
    );

    let output = Preprocessor::default().preprocess(
        SourceFile::from_path(workspace_fixture(
            "tests/fixtures/pp/list_fields_destination_root.prg",
        ))
        .expect("fixture"),
    );
...
#[test]
fn phase15_list_fields_off_reordered_fixture_matches_curated_upstream_subset() {
    let Some(upstream_pp_test) = read_upstream_or_skip(
        "harbour-core/tests/hbpp/_pp_test.prg",
        "upstream _pp_test corpus",
    ) else {
    ...
    let expected = fs::read_to_string(workspace_fixture(
        "tests/fixtures/pp/list_fields_off_reordered_root.out",
    ))
    .expect("fixture snapshot");
    ...
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a TO PRINTER OFF")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a TO FILE a OFF")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a,b TO PRINTER OFF")
    );
    assert!(
        upstream_pp_test
            .lines()
            .any(|line| line == "LIST a,b,(seek(a+b),c) TO FILE a OFF")
    );

    let output = Preprocessor::default().preprocess(
        SourceFile::from_path(workspace_fixture(
            "tests/fixtures/pp/list_fields_off_reordered_root.prg",
        ))
        .expect("fixture"),
    );
