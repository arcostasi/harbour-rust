...
| Runtime | baseline alpha amplo | valores centrais, arrays, builtins selecionados de string/matemática/conversão, cobertura de edge cases de strings guiada por oráculo para trim, busca, recorte e replicação, limites de overflow de string ao estilo Clipper em `Replicate()`/`Space()` e cobertura executável de `SubStr()`/`Right()` para o subconjunto atual sem `Chr(0)` |
...
- o caminho executável em C ainda trata strings como C strings, então a preservação de `Chr(0)` embutido segue como limitação documentada fora da surface do runtime em Rust;
