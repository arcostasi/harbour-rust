...
- casos avançados de pré-processador ainda permanecem em expansão para combinações mais amplas de dumb-stringify, expansão nested de opcionais/listas além do subconjunto focado atual de expansão de resultado `SET`, comportamento mais amplo de pattern markers de macro além do subconjunto atual `<id:&>` e semântica mais ampla de marcadores como `<{id}>`, `<"id">` e `<(id)>` em cenários com múltiplas expressões, strings e macros;
