...
| arrays | Clipper | partial | 7 | `src/vm/arrays.c` | parser + hir + runtime | `Value::Array` e construtores básicos já existem no runtime; literais `{}` e `{ expr, ... }` já parseiam; `expr[expr]`, `expr[expr, ...]` e encadeamento como `expr[expr][expr]` agora parseiam e baixam para HIR explícita na Fase 7; o runtime já expõe leitura 1-based com diagnóstico de bounds e tipo; a IR ainda emite placeholder estável, e escrita por índice, comparação e codegen seguem pendentes |
