...
| `STATIC` | Clipper | partial | 7 | `doc/statics.txt`, `tests/statics*.prg` | parser + hir + sema | `STATIC` já parseia com lista inicial de inicializadores, baixa para HIR como `Statement::Static`, e leituras nominais já entram como `Read(path)` explícito para preparar runtime/codegen; a sema ainda emite diagnóstico de storage não suportado, e storage persistente, runtime e codegen continuam pendentes |
