...
| operadores básicos | Clipper | partial | 1/2/3/4/5/7 | `ppcore.c`, `harbour.y` | unit + parser + sema + runtime + cli run | tokenização base e multi-caractere cobertas na Fase 1; parsing e resolução básica cobertas até a Fase 3; aritmética e comparação básicas cobertas no runtime da Fase 4; `<`, `<=`, `+` e `x++` entram no caminho executável inicial da Fase 5; `+= -= *= /= %= ^=` já parseiam para identificador simples na Fase 7 |
...
| arrays | Clipper | partial | 7 | `src/vm/arrays.c` | parser + hir + runtime | `Value::Array` e construtores básicos já existem no runtime; literais `{}` e `{ expr, ... }` já parseiam, baixam para HIR explícita e geram diagnóstico estável na IR; indexação, mutação, comparação e codegen ainda pendentes |
