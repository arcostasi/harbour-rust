...
| arrays | Clipper | partial | 7 | `src/vm/arrays.c`, `utils/hbtest/rt_hvm.prg`, `utils/hbtest/rt_hvma.prg`, `utils/hbtest/rt_array.prg` | parser + hir + runtime + compat + cli run | `Value::Array` e construtores básicos já existem no runtime; literais `{}` e `{ expr, ... }` já parseiam e agora baixam para HIR e IR explícitas; `expr[expr]`, `expr[expr, ...]` e encadeamento como `expr[expr][expr]` também já baixam para HIR e IR explícitas na Fase 7; o runtime Rust já expõe leitura e escrita 1-based com diagnóstico de bounds, tipo e caminho vazio; o backend C agora emite `harbour_value_from_array_items(...)`, `harbour_value_array_get(...)`, `harbour_value_array_set_path(...)`, `harbour_value_exact_equals(...)`, `harbour_value_equals(...)`, `harbour_value_not_equals(...)`, `harbour_value_greater_than(...)` e `harbour_value_greater_than_or_equal(...)`, e o suporte host já materializa `Array`, leitura e escrita indexadas 1-based, identidade observável de arrays para `==` e mensagens observáveis `BASE 1071/1072/1075/1076` no caminho `cli run`; `exact_equals()` já modela o baseline observável de `==` para arrays por identidade do valor observado, `=`/`<>` e ordenação com arrays agora já usam mensagens/códigos mais próximos do baseline xBase (`1071` a `1076`), e o fixture focado `tests/fixtures/compat/array_comparison_runtime.prg` agora fixa esse baseline diretamente contra `utils/hbtest/rt_hvm.prg` na surface pública do runtime; `tests/fixtures/parser/array_exact_compare.prg` agora também valida `==` de arrays no caminho executável `cli run`, e `tests/fixtures/parser/compare_ops.prg` valida os diagnósticos executáveis de `=`/`<>`/`>`/`>=`; `aadd()` e `asize()` já existem sobre `array_push()`/`array_resize()` via `call_builtin_mut()`, e `aclone()` agora expõe `AClone()` na surface imutável sobre `array_clone()`; `AClone()` já atravessa `codegen-c` e `cli run` via `harbour_builtin_aclone(...)`, e agora `AAdd()`/`ASize()` também atravessam `codegen-c` e `cli run` quando o primeiro argumento é símbolo endereçável simples, via `harbour_builtin_aadd(&value, ...)` e `harbour_builtin_asize(&value, ...)`; primeiros argumentos não endereçáveis continuam com erro explícito de codegen nesta fase; leitura e escrita indexadas agora já usam mensagens/códigos mais próximos do baseline xBase (`1068/1069/1132/1133`); `<` e `<=` com arrays ainda não têm fixture executável dedicada no host C, e semântica mais ampla para objetos, codeblocks e outras combinações continua pendente |
