...
| Preprocessor | curated advanced subset | `#define`, `#include`, `#command`, `#translate`, plus oracle-backed coverage for escaped optional replacements, selected optional-clause reordering, selected logical result markers like `<.id.>`, selected quoted result markers like `<"id">`, and a minimal smart-result subset for `<(id)>` |
...
- advanced preprocessor cases still remain in nested optional/list expansion, broader dumb-stringify combinations, and advanced marker forms beyond the current oracle-backed subset such as `<{id}>`, broader `<"id">` semantics around strings and macros, and broader `<(id)>` smart-result behavior around macros and quoted literals;
