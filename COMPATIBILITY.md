...
| arrays | Clipper | partial | 7 | `src/vm/arrays.c` | parser + hir + runtime | `Value::Array` e construtores básicos já existem no runtime; literais `{}` e `{ expr, ... }` já parseiam; `expr[expr]`, `expr[expr, ...]` e encadeamento como `expr[expr][expr]` agora parseiam e baixam para HIR explícita na Fase 7; o runtime Rust já expõe leitura 1-based com diagnóstico de bounds e tipo; o backend C já declara helpers de array e o suporte host já materializa `Array`, `array_len` e `array_get`; lowering executável de indexação, escrita por índice, comparação e codegen completo seguem pendentes |
