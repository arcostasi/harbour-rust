...
| Preprocessor | curated advanced subset | `#define`, `#include`, `#command`, `#translate`, plus oracle-backed coverage for escaped optional replacements, selected optional-clause reordering, and selected logical result markers like `<.id.>` |
...
- advanced preprocessor cases still remain in nested optional/list expansion, broader dumb-stringify combinations, and advanced marker forms beyond the current oracle-backed subset such as `<(id)>`, `<"id">`, and `<{id}>`;
