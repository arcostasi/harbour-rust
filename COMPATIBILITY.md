...
| `STATIC` | Clipper | partial | 7 | `doc/statics.txt`, `tests/statics*.prg` | parser + hir + sema + ir + codegen-c | `STATIC` já parseia com lista inicial de inicializadores, baixa para HIR e IR como `Statement::Static`, e leituras nominais já entram como `Read(path)` explícito nas duas camadas; o `codegen-c` agora gera storage C persistente por rotina com guard de inicialização para leituras/escritas nominais no mesmo routine; a sema ainda emite diagnóstico de storage não suportado, então o caminho completo via CLI continua pendente, assim como storage compartilhado entre rotinas e suporte de runtime mais amplo |
