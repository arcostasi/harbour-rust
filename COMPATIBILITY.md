...
- advanced preprocessor cases still remain in broader nested optional/list expansion beyond the current focused `AAA`/`SET`/`AVG`/`INSERT`/`INSERT2`/`MYCOMMAND3` multiline-reorder/multiline-result subset, now including the multiline `SET`/`AVG` declarations exercised in `hbpptest.prg`, broader dumb-stringify combinations beyond the current `_DUMB_M(<z>)` string/quoted-literal subset, broader pattern-level macro marker behavior beyond the current expanded `<id:&>` subset, and broader marker semantics beyond the current oracle-backed subset such as multi-expression `<{id}>` blockify behavior and additional `<"id">`/`<(id)>` handling beyond the current `_NORMAL_M(<z>)` and `_SMART_M(<z>)` string/quoted-literal subsets and the existing macro-oriented coverage;
