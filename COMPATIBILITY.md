...
| `STATIC` | Clipper | partial | 7 | `doc/statics.txt`, `tests/statics*.prg` | parser + hir + sema + ir | `STATIC` já parseia com lista inicial de inicializadores, baixa para HIR e IR como `Statement::Static`, e leituras nominais já entram como `Read(path)` explícito nas duas camadas para preparar runtime/codegen; a sema ainda emite diagnóstico de storage não suportado, e o `codegen-c` agora também falha com erro explícito para `STATIC` em vez de achatá-lo silenciosamente; storage persistente, runtime e codegen real continuam pendentes |
