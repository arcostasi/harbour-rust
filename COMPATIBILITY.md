...
- advanced preprocessor cases still remain in nested optional/list expansion, broader dumb-stringify combinations, and broader marker semantics beyond the current oracle-backed subset such as multi-expression `<{id}>` blockify behavior and broader `<"id">`/`<(id)>` handling around strings, macros, and quoted literals;
